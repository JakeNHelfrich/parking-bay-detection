...
**Confirmed transitions (debounce + hysteresis).** Per-frame occupancy can flicker on noisy detections, so the frontend only reports a transition once the new state has held for a time-based debounce — in *seconds*, not frames, because capture rate varies (`src/bays/stabilizer.ts`). Hysteresis makes the holds directional: entering `occupied` requires the truck to be seen for `ENTER_OCCUPIED_SECS` (2s); leaving it requires absence to hold `LEAVE_OCCUPIED_SECS` (4s), so near-miss maneuvers and momentary detection dropouts never flip a state. Bays start implicitly empty — "still empty" is never reported — and a batch dropped while the socket is down is not re-sent after reconnect (the stabilizer keeps its confirmed state, so no duplicates). The same confirmed states (each stamped with a `sinceMs` set only on confirmed transitions) feed the store snapshot and the board, so the UI cannot flicker independently of the record. Bay cards render the duration of the current state ("for 6 min", "1 h 12 m") from that stamp, recomputed on every store emit — no per-component timers (`src/ui/duration.ts`; the full-viewport board view reuses the same helpers).

**Honest "can't confirm" (trust).** Stability is not the same as truth: the board only asserts a bay state while the evidence stream backs it (`src/bays/trust.ts`). Every bay reads an explicit gray "Can't confirm" — with the reason — when the sim is idle, the socket is offline, no detections frame has arrived yet, or the last frame is older than `DETECTIONS_STALE_AFTER_MS` (5s; captures run ~10 fps, so this rides out normal backpressure jitter). A matched truck below `BAY_TRUST_MIN_CONFIDENCE` (0.35) shows "Can't confirm · weak truck match", and a bay never observed says so instead of guessing "Clear". The overlay mirrors this: unconfirmed bays draw gray with a number-only label instead of the last-known OCC/CLEAR. Trust is a display projection only — the stabilizer and the server record keep reporting the last confirmed state (invariant 5); the last evidence timestamp (`detectionsAtMs`) is stamped publisher-side in the store, and the rAF-driven HUD publish keeps the staleness clock fresh even when detections stop, so the board never silently freezes on last-known data.

**Glanceable yard board (yp6.4).** The header's "Yard board" toggle overlays a full-viewport board above the live view (`src/ui/YardBoard.tsx`): one large color block per bay — a state word ("Full" / "Free"), the bay label, and the state duration — readable from across a room; tiles wrap responsively down to tablet width. The board reuses the detail cards' exact tone and copy rules (`bayTone`/`bayStatusCopy`/`bayDurationCopy` via the pure `yardTileCopy`), so the summary and detail views can never disagree, and the trust rule carries over: gray "Can't confirm" tiles whenever the evidence stream can't back a claim. It renders as a store-driven overlay (`boardOpen`) with the sim pipeline still mounted underneath, so the board stays live while open; the sidebar cards remain the detail view, and the overlay canvas keeps drawing only detection boxes and bay rects.
